...
class VectorService:
    ...
    def store_vector(self, asset_id: int, user_id: int, embedding: list):
        self.__vectorRepo.create_vector(asset_id=asset_id, user_id=user_id, embedding=embedding)

    def search_similar_vectors(self,user_id: int, query_embedding: list):
        return self.__vectorRepo.search_vector(user_id = user_id, query_embedding=query_embedding)
